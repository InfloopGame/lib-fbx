...
export function buildMeshGeometry(data: MeshGeometryData, preTransform?: Matrix4): BufferGeometry | null {
  ...
  // Tree conversion bakes geometric transforms; SDK conversion keeps them on the mesh.
  if (preTransform) geo.getAttribute('position').applyMatrix4(preTransform)
  if (nrm.length === pos.length) {
    const nAttr = new BufferAttribute(new Float32Array(nrm), 3)
    if (preTransform) nAttr.applyNormalMatrix(new Matrix3().getNormalMatrix(preTransform))
