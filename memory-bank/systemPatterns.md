...
- 入口：`src/index.ts` 再导出 detect / parse / parseWasm / FbxError / version。
...
- 测试：根目录 `tests/`（不放 `src`），vitest，覆盖率统计只含 `src`，排除 `src/types.ts` 与 `src/wasm/pkg/**` 与 `src/wasm/inline.ts`。
...
- 公共 API：`parseWasm(input): Promise<FbxDocument>` 与 `ensureWasmReady()`，前者对 binary 走 wasm 后端，ascii 抛 `UNKNOWN_FORMAT`。
